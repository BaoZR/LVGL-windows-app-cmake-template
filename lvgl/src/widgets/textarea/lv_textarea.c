...
void lv_textarea_copy_text_to_clipboard(lv_obj_t* obj)
{
    ...
    if (hMem != NULL)
    {
        char* lpMem = GlobalLock(hMem);
        if(lpMem != NULL) strcpy(lpMem, text);
        GlobalUnlock(hMem);
...
void lv_textarea_paste_text_from_clipboard(lv_obj_t* obj)
{
#ifdef _WINDOWS
    if (!OpenClipboard(NULL)) return;
    ...
    if (hglb != NULL) {
        ...
        if (lptstr != NULL) {
            lv_textarea_add_text(obj, lptstr);
            GlobalUnlock(hglb);
        }
    }
    CloseClipboard();
...
static void lv_textarea_event(const lv_obj_class_t * class_p, lv_event_t * e)
{
    ...
    if(code == LV_EVENT_FOCUSED) {
        ...
        if (c == LV_KEY_RIGHT)
            ...
            lv_textarea_copy_text_to_clipboard(obj);
        else if (c == LV_KEY_PASTE)
            lv_textarea_paste_text_from_clipboard(obj);
